...
{
	[Serializable]
	[Description("Dual MACD Indicator based on ALMA")]
	...
	public class FTR : IndicatorBase
	{
		...
		internal class F_TR
		{
			...
			//RMSobj RMSFast;
			//ALMAobj MACD_1_ALMASlow;
			//ALMAobj MACD_1_ALMAFast;
			ALMAobj FTR_ALMA;
			//ALMAobj MACD_2_ALMAFast;
			//ALMAobj MACD2_SIGNAL;
			FTRobj FTR;
			...
			//internal double macd1;
			//internal double macd2;
			//internal double macd2_prev1;
			//internal double rms;
			//internal double rms_upper;
			//internal double rms_lower;
			//internal double rms_factor;
			//internal double macd2_alma;
			internal double ftr;
			...
			//Variables required for 3-Pole Super Smoother
			//internal int macd2smoothperiod;
			//internal double a1;
			//internal double b1;
			//internal double c1;
			//internal double coef2;
			...
			//internal double coef4;
			//internal double coef1;
			//internal double macd2smooth;
			//internal double macd2smooth_prev1;
			//internal double macd2smooth_prev2;
			//internal double macd2smooth_prev3;

			//indicator state variables
			internal bool ftr_low;
			//internal bool macdlow;
			//internal bool macdmidhigh;
			//internal bool macdmidlow;
			//internal bool macd_oposite_macd1high;
			//internal bool macd_oposite_macd2high;
			...
			internal F_TR(FTR eaIn, int FTR_Period, int ALMA_Period) : this()
			{
				...
				//RMSFast_Upper = new RMSobj(RMS_Period, RMS_Smooth_Period);
				//RMSFast_Lower = new RMSobj(RMS_Period, RMS_Smooth_Period);
				//RMSFast = new RMSobj(RMS_Period, RMS_Smooth_Period);
				//MACD_1_ALMASlow = new ALMAobj(MACD_1_Slow_Period, SlopeThreshold);
				//MACD_1_ALMAFast = new ALMAobj(MACD_1_Fast_Period, SlopeThreshold);
				FTR_ALMA = new ALMAobj(ALMA_Period, Threshold);
				//MACD_2_ALMAFast = new ALMAobj(MACD_2_Fast_Period, SlopeThreshold);
				//MACD2_SIGNAL = new ALMAobj(MACD_2_Signal_Period, SlopeThreshold);
				...
				// initialise 3-Pole Super Smoother Variables
				//macd2smoothperiod = MACD_2_Smooth_Period;
				//a1 = Math.Exp(-3.1459 / macd2smoothperiod);
				//b1 = 2 * a1 * Math.Cos(Math.PI * (1.738 * 180 / macd2smoothperiod) / 180.0);
				//c1 = a1 * a1;
				//coef2 = b1 + c1;
				//coef3 = -(c1 + b1 * c1);
				//coef4 = c1 * c1;
...

			{
				...
				//MACD_1_ALMAFast.Init(priceAvg);
				//MACD_1_ALMASlow.Init(priceAvg);
				//MACD_2_ALMAFast.Init(priceAvg);
				FTR_ALMA.Init(priceAvg);
				//RMSFast_Upper.Init(priceAvg);
				//RMSFast_Lower.Init(priceAvg);
				//RMSFast.Init(priceAvg);
				//MACD2_SIGNAL.Init(priceAvg);
				FTR.Init(b);
			...
			internal void Calc(Bar theBar)
			{
				...
				//macd calculation
				//MACD_1_ALMAFast.Calc(priceAvg);
				//MACD_1_ALMASlow.Calc(priceAvg);
				//MACD_2_ALMAFast.Calc(priceAvg);
				//MACD_2_ALMASlow.Calc(priceAvg);
				//macd1 = MACD_1_ALMAFast.value - MACD_1_ALMASlow.value;
				//macd2 = MACD_2_ALMAFast.value - MACD_2_ALMASlow.value;
				//rms = RMSFast.Calc( Math.Abs(macd1) );
				ftr = FTR.Calc(theBar);
				//ftr_alma = FTR_ALMA.Calc(ftr);
				ftr_alma = FTRDecycle.Calc(ftr);

				//macd2smooth = coef1 * macd2 + coef2 * macd2smooth_prev1 + coef3 * macd2smooth_prev2 + coef4 * macd2smooth_prev3;

				//macd2_alma = MACD2_SIGNAL.Calc(macd2smooth);

				//rms calculation
				//if (macd1 >= 0)
				//{
				//	rms_upper = rms_factor * RMSFast_Upper.Calc((Math.Abs(macd1)));
				//	RMSFast_Lower.Calc(0);
				//	rms_lower = 0;
				//}
				//if (macd1 < 0)
				//{
				//	rms_lower = -rms_factor * RMSFast_Lower.Calc((Math.Abs(macd1)));
				//	RMSFast_Upper.Calc(0);
				//	rms_upper = 0;
				//}

				//indicator state
				//macd2isRising = (macd2smooth >= macd2smooth_prev1);
				ftrisRising = (ftr >= ftr_alma);
				//macd2isFalling = (macd2smooth < macd2smooth_prev1);
				...
				//macd2smooth_prev3 = macd2smooth_prev2;
				//macd2smooth_prev2 = macd2smooth_prev1;
				//macd2smooth_prev1 = macd2smooth;
