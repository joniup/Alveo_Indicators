...
namespace Alveo.UserCode
{
    ...
    public class ALMA_ADAPTIVE : IndicatorBase
    {
        #region Properties
        ...
        public int IndPeriod_Min { get; set; }
		...
		[DisplayName("MA Period. ")]
		[Description("Sets the strength of the filtering. [ex: 20]")]
		public int IndPeriod_Max { get; set; }
		...
		[Category("Settings")]
		[DisplayName("Fractal Lookback Period. ")]
		[Description("Sets the strength of the filtering. [ex: 20]")]
		public int Fractal_Lookback_Period { get; set; }
		...
		//variables for Fractal Calculation
		internal double HH;
		internal double LL;
		internal double prevHH;
		internal double prevLL;
		internal double FractalDimension;
		internal double Period_Adapt;
		internal double Adapt_Factor_Fish;
		internal double Adapt_Factor;

		/// <summary>  
		///  C# constructor for ALMA Class
		///  called to initialize the class when class is created by Alveo
		/// </summary>  
		public ALMA_ADAPTIVE()
        {
            try
            {
                ...
                IndPeriod_Min = 50;                     // Initial value for DTEMAv2 period
				IndPeriod_Max = 700;                     // Initial value for DTEMAv2 period
				SlopeThreshold = 10;                // Initial value for slopeThreshold
                Offset = 0.15;
				Fractal_Lookback_Period = 20;
        ...
        protected override int Init()
        {
            try
            {
                ...
                SetIndexLabel(0, "ALMA_Adp(" + IndPeriod_Min + "/" + IndPeriod_Max + ").Bull");

                SetIndexStyle(1, DRAW_LINE, STYLE_SOLID);       // repeat for all 3 buffers
                SetIndexLabel(1, "ALMA_Adp(" + IndPeriod_Min + "/" + IndPeriod_Max + ").Bear");

                SetIndexStyle(2, DRAW_LINE, STYLE_SOLID);
                SetIndexLabel(2, "ALMA_Adp(" + IndPeriod_Min + "/" + IndPeriod_Max + ").Mixed");

                // Sets the "short" name of a custom indicator to be shown in the DataWindow and in the chart subwindow.
                IndicatorShortName("ALMA_Adt (" + IndPeriod_Min + "," + IndPeriod_Max + ")");

                //DTema = new DTEMAobj(IndPeriod, SlopeThreshold);
                alma = new ALMA_ADAP_obj(IndPeriod_Max, SlopeThreshold, Offset);

                Print("ALMA_Adp: Started. [" + Chart.Symbol + "] tf=" + Period());      // Print this message to Alveo Log file on startup
            }
            catch (Exception ex)
            {
        ...
        protected override int Start()
        {
            try  // to catch and handle Exceptions that might occur in this code block
            {
                ...
                if (firstrun)   // on first run, calculate ALMA on all ChartBars data
                {
					...
					Period_Adapt = (IndPeriod_Max + IndPeriod_Min) / 2;
                ...
                else // not firstrun; only calculate DTEMA on ChartBars not already processed
                {
					...
					if (pos < (data.Count - Fractal_Lookback_Period))
					{
						prevHH = 0; prevLL = 1000;

						//Overall highest high and lowest low
						for (int i = 0; i < (Fractal_Lookback_Period); i++)
						{
							HH = Math.Max((double)data[pos + i].High, (double)prevHH);
							LL = Math.Min((double)data[pos + i].Low, (double)prevLL);
							prevHH = HH; prevLL = LL;
						}
						double N3 = (HH - LL) / Fractal_Lookback_Period;
						prevHH = 0; prevLL = 1000;


						//Highest high and lowest low of first quadrant
						for (int i = 0; i < (Fractal_Lookback_Period / 2); i++)
						{
							HH = (double)Math.Max((decimal)data[pos + i].High, (decimal)prevHH);
							LL = (double)Math.Min((decimal)data[pos + i].Low, (decimal)prevLL);
							prevHH = HH; prevLL = LL;
						}
						double N1 = (HH - LL) / (Fractal_Lookback_Period / 2);
						prevHH = 0; prevLL = 1000;

						//Highest high and lowest low of first quadrant
						for (int i = Fractal_Lookback_Period / 2; i < (Fractal_Lookback_Period); i++)
						{
							HH = (double)Math.Max((decimal)data[pos + i].High, (decimal)prevHH);
							LL = (double)Math.Min((decimal)data[pos + i].Low, (decimal)prevLL);
							prevHH = HH; prevLL = LL;
						}
						double N2 = (HH - LL) / (Fractal_Lookback_Period / 2);

						prevHH = 0; prevLL = 1000;

						if (N3 > 0 && N2 > 0 && N1 > 0)
						{
							FractalDimension = ((Math.Log(N1 + N2) - Math.Log(N3)) / Math.Log(2));
						}


						Adapt_Factor = (2 * (FractalDimension - 1)) - 1;
						Adapt_Factor_Fish = ((Math.Exp(2 * 2 * Adapt_Factor_Fish) - 1) / (Math.Exp(2 * 2 * Adapt_Factor) + 1) + 1) / 2;
						Period_Adapt = IndPeriod_Min + Adapt_Factor_Fish * (IndPeriod_Max - IndPeriod_Min);


					}
					else Period_Adapt = (IndPeriod_Max+IndPeriod_Min)/2;
                ...
                for (int i = 0; i < e; i++)     // iterate each bar to process  
                {
                    ...
                    alma.Calc(thePrice, Period_Adapt);                                        // calculate new ALMA value
        ...
        //Alma - moving average
        internal class ALMA_ADAP_obj
        {
            public double alma;
            public double wSum;
            private bool firstRun;
            Queue<double> Qprices;
			Queue<double> Qprices_Adapt;
            ...
            internal ALMA_ADAP_obj()
            {
				...
				Qprices_Adapt = new Queue<double>();
				prevState = 0;
                firstRun = true;
                isRising = false;
                isFalling = false;
            ...
            internal ALMA_ADAP_obj(double period, int threshold, double offset) : this()
            {
                Period = period;
                OFFset = offset;
            ...
            //Alma Calculation
            internal double Calc(double Price, double Adapt_Period)
            {
				...
				Qprices_Adapt = Qprices;

				if (Adapt_Period < Period)
				{
					while (Qprices_Adapt.Count > (Adapt_Period + 1))
					{
						Qprices_Adapt.Dequeue();
					}
				}

				var arr = Qprices_Adapt.ToArray();
                var count = arr.Count();
